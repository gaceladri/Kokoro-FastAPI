...
try:
    ...
    logger.info("Successfully imported stripe and supabase modules")
except ImportError as e:
    logger.error(f"Error importing modules: {e}")
    logger.error(f"Available modules: {sys.modules.keys()}")
    raise
...
logger.info(f"STRIPE_API_KEY present: {STRIPE_API_KEY is not None}")
logger.info(f"SUPABASE_URL present: {SUPABASE_URL is not None}")
